...
class Triggers:
  ...
  def check_goodbye(self, text: str) -> bool:
    return any(keyword in nltk.tokenize.word_tokenize(text.lower()) for keyword in Triggers.goodbye) or all(
      keyword in nltk.tokenize.word_tokenize(text.lower()) for keyword in Triggers.goodbye_default_phrase
    )
