...
class Synthesizer:
    def __init__(self, n_vocab, spec_channels, segment_size, inter_channels, hidden_channels, filter_channels, n_heads, n_layers, kernel_size, p_dropout, resblock, resblock_kernel_sizes, resblock_dilation_sizes, upsample_rates, upsample_initial_channel, upsample_kernel_sizes, n_speakers=0, gin_channels=0, use_sdp=True, emotion_embedding=False, **kwargs):
        ...
        self.enc_p = TextEncoder(n_vocab, inter_channels, hidden_channels, filter_channels,
                                 n_heads, n_layers, kernel_size, p_dropout, emotion_embedding)
        ...
        self.dp = StochasticDurationPredictor(hidden_channels, 192, 3, 0.5, 4, gin_channels=gin_channels) if use_sdp else DurationPredictor(
    ...
    def infer(self, x, x_lengths, sid=None, noise_scale=1.0, length_scale=1, noise_scale_w=1., max_len=None, emotion_embedding=None, max_y_length_estimate_scale=None, pad_length=-1):
        x, m_p, logs_p, x_mask = self.enc_p.forward(x.realize(), x_lengths.realize(
        ), emotion_embedding.realize() if emotion_embedding is not None else emotion_embedding)
        g = self.emb_g(sid.reshape(1, 1)).squeeze(
            1).unsqueeze(-1) if self.n_speakers > 0 else None
        logw = self.dp.forward(x, x_mask.realize(), g=g.realize(
        ), reverse=self.use_sdp, noise_scale=noise_scale_w if self.use_sdp else 1.0)
    ...
    def generate(self, g, logs_p, m_p, max_len, max_y_length_estimate_scale, noise_scale, w_ceil, x, x_mask, y_lengths, pad_length):
        max_y_length = y_lengths.max().item() if max_y_length_estimate_scale is None else max(
            15, x.shape[-1]) * max_y_length_estimate_scale
        y_mask = sequence_mask(y_lengths, max_y_length).unsqueeze(
            1).cast(x_mask.dtype)
        attn_mask = x_mask.unsqueeze(2) * y_mask.unsqueeze(-1)
        attn = generate_path(w_ceil, attn_mask)
        m_p_2 = attn.squeeze(1).matmul(m_p.transpose(1, 2)).transpose(
            1, 2)        # [b, t', t], [b, t, d] -> [b, d, t']
        logs_p_2 = attn.squeeze(1).matmul(logs_p.transpose(1, 2)).transpose(
            1, 2)  # [b, t', t], [b, t, d] -> [b, d, t']
        z_p = m_p_2 + \
            Tensor.randn(*m_p_2.shape, dtype=m_p_2.dtype) * \
            logs_p_2.exp() * noise_scale
        row_len = y_mask.shape[2]
        if pad_length > -1:
            # Pad flow forward inputs to enable JIT
            assert pad_length > row_len, "pad length is too small"
            y_mask = y_mask.pad(
                ((0, 0), (0, 0), (0, pad_length - row_len)), 0).cast(z_p.dtype)
            # New y_mask tensor to remove sts mask
            y_mask = Tensor(y_mask.numpy(), device=y_mask.device,
                            dtype=y_mask.dtype, requires_grad=y_mask.requires_grad)
            z_p = z_p.squeeze(0).pad(
                ((0, 0), (0, pad_length - z_p.shape[2])), 1).unsqueeze(0)
        z = self.flow.forward(
            z_p.realize(), y_mask.realize(), g=g.realize(), reverse=True)
        result_length = reduce(
            lambda x, y: x * y, self.dec.upsample_rates, row_len)
        o = self.dec.forward(
            (z * y_mask)[:, :, :max_len], g=g)[:, :, :result_length]
        if max_y_length_estimate_scale is not None:
            ...
            if real_max_y_length < max_y_length:
                overestimation = (
                    (max_y_length / real_max_y_length) * 100) - 100
                logging.info(
                    f"Overestimated max length by {overestimation:.2f}%")
...
class StochasticDurationPredictor:
    def __init__(self, in_channels, filter_channels, kernel_size, p_dropout, n_flows=4, gin_channels=0):
        ...
        self.post_pre, self.post_proj = nn.Conv1d(
            1, filter_channels, 1), nn.Conv1d(filter_channels, filter_channels, 1)
    ...
    def forward(self, x: Tensor, x_mask, w=None, g=None, reverse=False, noise_scale=1.0):
        ...
        if not reverse:
            ...
            log_det_tot_q = 0
            h_w = self.post_proj(self.post_convs.forward(
                self.post_pre(w), x_mask)) * x_mask
            e_q = Tensor.randn(w.size(0), 2, w.size(
                2), dtype=x.dtype).to(device=x.device) * x_mask
            z_q = e_q
            ...
            z0 = (w - u) * x_mask
            log_det_tot_q += Tensor.sum((z_u.logsigmoid() +
                                        (-z_u).logsigmoid()) * x_mask, [1, 2])
            log_q = Tensor.sum(-0.5 * (math.log(2*math.pi) +
                               (e_q**2)) * x_mask, [1, 2]) - log_det_tot_q
            log_det_tot = 0
            ...
            nll = Tensor.sum(0.5 * (math.log(2*math.pi) + (z**2))
                             * x_mask, [1, 2]) - log_det_tot
            return (nll + log_q).realize()  # [b]
        flows = list(reversed(self.flows))
        flows = flows[:-2] + [flows[-1]]  # remove a useless vflow
        z = Tensor.randn(x.shape[0], 2, x.shape[2], dtype=x.dtype).to(
            device=x.device) * noise_scale
        for flow in flows:
...
class DurationPredictor:
    def __init__(self, in_channels, filter_channels, kernel_size, p_dropout, gin_channels=0):
        ...
        self.conv_1, self.norm_1 = nn.Conv1d(
            in_channels, filter_channels, kernel_size, padding=kernel_size//2), LayerNorm(filter_channels)
        self.conv_2, self.norm_2 = nn.Conv1d(
            filter_channels, filter_channels, kernel_size, padding=kernel_size//2), LayerNorm(filter_channels)
...
class TextEncoder:
    ...
    def forward(self, x: Tensor, x_lengths: Tensor, emotion_embedding=None):
        ...
        x_mask = sequence_mask(
            x_lengths, x.shape[2]).unsqueeze(1).cast(x.dtype)
...
class PosteriorEncoder:
    ...
    def forward(self, x, x_lengths, g=None):
        ...
        stats = self.proj(self.enc.forward(
            self.pre(x) * x_mask, x_mask, g=g)) * x_mask
...
class Generator:
    def __init__(self, initial_channel, resblock, resblock_kernel_sizes, resblock_dilation_sizes, upsample_rates, upsample_initial_channel, upsample_kernel_sizes, gin_channels=0):
        self.num_kernels, self.num_upsamples = len(
            resblock_kernel_sizes), len(upsample_rates)
        self.conv_pre = nn.Conv1d(
            initial_channel, upsample_initial_channel, 7, 1, padding=3)
        resblock = ResBlock1 if resblock == '1' else ResBlock2
        self.ups = [nn.ConvTranspose1d(upsample_initial_channel//(2**i), upsample_initial_channel//(2**(
            i+1)), k, u, padding=(k-u)//2) for i, (u, k) in enumerate(zip(upsample_rates, upsample_kernel_sizes))]
        self.resblocks = []
    ...
    def forward(self, x: Tensor, g=None):
        ...
        for i in range(self.num_upsamples):
            ...
            xs = sum(self.resblocks[i * self.num_kernels + j].forward(x)
...
class LayerNorm(nn.LayerNorm):
    def __init__(self, channels, eps=1e-5): super().__init__(channels,
                                                             eps, elementwise_affine=True)
    def forward(self, x: Tensor): return self.__call__(
        x.transpose(1, -1)).transpose(1, -1)
...
class WN:
    def __init__(self, hidden_channels, kernel_size, dilation_rate, n_layers, gin_channels=0, p_dropout=0):
        ...
        for i in range(n_layers):
            ...
            self.in_layers.append(nn.Conv1d(hidden_channels, 2 * hidden_channels, kernel_size,
                                  dilation=dilation, padding=int((kernel_size * dilation - dilation) / 2)))
...
class ResBlock1:
    def __init__(self, channels, kernel_size=3, dilation=(1, 3, 5)):
        self.convs1 = [nn.Conv1d(channels, channels, kernel_size, 1, dilation=dilation[i], padding=get_padding(
            kernel_size, dilation[i])) for i in range(3)]
        self.convs2 = [nn.Conv1d(channels, channels, kernel_size, 1, dilation=1, padding=get_padding(
            kernel_size, 1)) for _ in range(3)]

    def forward(self, x: Tensor, x_mask=None):
        for c1, c2 in zip(self.convs1, self.convs2):
            xt = x.leakyrelu(LRELU_SLOPE)
            xt = c1(xt if x_mask is None else xt *
                    x_mask).leakyrelu(LRELU_SLOPE)
...
class ResBlock2:
    def __init__(self, channels, kernel_size=3, dilation=(1, 3)):
        self.convs = [nn.Conv1d(channels, channels, kernel_size, 1, dilation=dilation[i],
                                padding=get_padding(kernel_size, dilation[i])) for i in range(2)]
    ...
    def __init__(self, channels, kernel_size, n_layers, p_dropout=0.):
        ...
        for i in range(n_layers):
            ...
            self.convs_sep.append(nn.Conv1d(
                channels, channels, kernel_size, groups=channels, dilation=dilation, padding=padding))
...
class ConvFlow:
    ...
    def forward(self, x, x_mask, g=None, reverse=False):
        ...
        un_normalized_widths = h[..., :self.num_bins] / \
            math.sqrt(self.filter_channels)
        un_normalized_heights = h[..., self.num_bins:2 *
...
class ResidualCouplingLayer:
    ...
    def forward(self, x, x_mask, g=None, reverse=False):
        x0, x1 = split(x, [self.half_channels] * 2, 1)
        stats = self.post(self.enc.forward(
            self.pre(x0) * x_mask, x_mask, g=g)) * x_mask
...
class ElementwiseAffine:
    def __init__(self, channels): self.m, self.logs = Tensor.zeros(
        channels, 1), Tensor.zeros(channels, 1)
...
class MultiHeadAttention:
    def __init__(self, channels, out_channels, n_heads, p_dropout=0., window_size=None, heads_share=True, block_length=None, proximal_bias=False, proximal_init=False):
        ...
        if window_size is not None:
            self.emb_rel_k, self.emb_rel_v = [Tensor.randn(
                1 if heads_share else n_heads, window_size * 2 + 1, self.k_channels) * (self.k_channels ** -0.5) for _ in range(2)]
    ...
    def attention(self, query: Tensor, key: Tensor, value: Tensor, mask=None):
        ...
        query = query.reshape(
            b, self.n_heads, self.k_channels, t_t).transpose(2, 3)
        key = key.reshape(b, self.n_heads, self.k_channels,
                          t_s).transpose(2, 3)
        value = value.reshape(
            b, self.n_heads, self.k_channels, t_s).transpose(2, 3)
        ...
        if self.window_size is not None:
            ...
            scores = scores + \
                self._relative_position_to_absolute_position(rel_logits)
        if mask is not None:
            scores = Tensor.where(mask, scores, -1e4)
            if self.block_length is not None:
                assert t_s == t_t, "Local attention is only available for self-attention."
                scores = Tensor.where(Tensor.ones_like(
                    scores).triu(-self.block_length).tril(self.block_length), scores, -1e4)
        ...
        output = output.transpose(2, 3).contiguous().reshape(
            b, d, t_t)  # [b, n_h, t_t, d_k] -> [b, d, t_t]
        return output, p_attn

    # x: [b, h, l, m], y: [h or 1, m, d], ret: [b, h, l, d]
    def _matmul_with_relative_values(
        self, x, y): return x.matmul(y.unsqueeze(0))

    def _matmul_with_relative_keys(self, x, y): return x.matmul(y.unsqueeze(
        0).transpose(-2, -1))  # x: [b, h, l, d], y: [h or 1, m, d], re, : [b, h, l, m]

    def _get_relative_embeddings(self, relative_embeddings, length):
        pad_length, slice_start_position = max(
            length - (self.window_size + 1), 0), max((self.window_size + 1) - length, 0)
        ...
        return padded_relative_embeddings[:, slice_start_position:(slice_start_position + 2 * length - 1)]
    ...
    def _relative_position_to_absolute_position(self, x: Tensor):
        ...
        x_flat = x.reshape([batch, heads, length * 2 * length]
                           ).pad(convert_pad_shape([[0, 0], [0, 0], [0, length-1]]))
    ...
    def _absolute_position_to_relative_position(self, x: Tensor):
        ...
        x_flat = x.reshape([batch, heads, length**2 + length*(length - 1)]
                           ).pad(convert_pad_shape([[0, 0], [0, 0], [length, 0]]))
...
class FFN:
    def __init__(self, in_channels, out_channels, filter_channels, kernel_size, p_dropout=0., activation=None, causal=False):
        ...
        self.conv_1, self.conv_2 = nn.Conv1d(in_channels, filter_channels, kernel_size), nn.Conv1d(
            filter_channels, out_channels, kernel_size)
    ...
    def _causal_padding(self, x): return x if self.kernel_size == 1 else x.pad(
        convert_pad_shape([[0, 0], [0, 0], [self.kernel_size - 1, 0]]))
    def _same_padding(self, x): return x if self.kernel_size == 1 else x.pad(
        convert_pad_shape([[0, 0], [0, 0], [(self.kernel_size - 1) // 2, self.kernel_size // 2]]))
...
class Encoder:
    ...
    def forward(self, x, x_mask):
        ...
        for i in range(self.n_layers):
            y = self.attn_layers[i].forward(
                x, x, attn_mask).dropout(self.p_dropout)
...
def piecewise_rational_quadratic_transform(inputs, un_normalized_widths, un_normalized_heights, un_normalized_derivatives, inverse=False, tails=None, tail_bound=1., min_bin_width=DEFAULT_MIN_BIN_WIDTH, min_bin_height=DEFAULT_MIN_BIN_HEIGHT, min_derivative=DEFAULT_MIN_DERIVATIVE):
    if tails is None:
        ...
        spline_fn, spline_kwargs = unconstrained_rational_quadratic_spline, {
            'tails': tails, 'tail_bound': tail_bound}
...
def unconstrained_rational_quadratic_spline(inputs, un_normalized_widths, un_normalized_heights, un_normalized_derivatives, inverse=False, tails='linear', tail_bound=1., min_bin_width=DEFAULT_MIN_BIN_WIDTH, min_bin_height=DEFAULT_MIN_BIN_HEIGHT, min_derivative=DEFAULT_MIN_DERIVATIVE):
    ...
    un_normalized_derivatives = cat_lr(
        un_normalized_derivatives, constant, constant)
    output, log_abs_det = rational_quadratic_spline(inputs=inputs.squeeze(dim=0).squeeze(dim=0), unnormalized_widths=un_normalized_widths.squeeze(dim=0).squeeze(dim=0), unnormalized_heights=un_normalized_heights.squeeze(dim=0).squeeze(
        dim=0), unnormalized_derivatives=un_normalized_derivatives.squeeze(dim=0).squeeze(dim=0), inverse=inverse, left=-tail_bound, right=tail_bound, bottom=-tail_bound, top=tail_bound, min_bin_width=min_bin_width, min_bin_height=min_bin_height, min_derivative=min_derivative)
...
def rational_quadratic_spline(inputs: Tensor, unnormalized_widths: Tensor, unnormalized_heights: Tensor, unnormalized_derivatives: Tensor, inverse=False, left=0., right=1., bottom=0., top=1., min_bin_width=DEFAULT_MIN_BIN_WIDTH, min_bin_height=DEFAULT_MIN_BIN_HEIGHT, min_derivative=DEFAULT_MIN_DERIVATIVE):
    ...
    widths = min_bin_width + (1 - min_bin_width * num_bins) * \
        unnormalized_widths.softmax(axis=-1)
    cum_widths = cat_lr(((right - left) * widths[..., :-1].cumsum(
        axis=1) + left), left, right + 1e-6 if not inverse else right)
    widths = cum_widths[..., 1:] - cum_widths[..., :-1]
    derivatives = min_derivative + (unnormalized_derivatives.exp()+1).log()
    heights = min_bin_height + \
        (1 - min_bin_height * num_bins) * unnormalized_heights.softmax(axis=-1)
    cum_heights = cat_lr(((top - bottom) * heights[..., :-1].cumsum(
        axis=1) + bottom), bottom, top + 1e-6 if inverse else top)
    heights = cum_heights[..., 1:] - cum_heights[..., :-1]
    bin_idx = ((inputs[..., None] >= (cum_heights if inverse else cum_widths)).sum(
        axis=-1) - 1)[..., None]
    ...
    input_derivatives_plus_one = gather(
        derivatives[..., 1:], bin_idx, axis=-1)[..., 0]
    input_heights = gather(heights, bin_idx, axis=-1)[..., 0]
    if inverse:
        a = ((inputs - input_cum_heights) * (input_derivatives + input_derivatives_plus_one -
        ...
        derivative_numerator = input_delta.square() * (input_derivatives_plus_one * root.square() +
                                                       2 * input_delta * theta_one_minus_theta + input_derivatives * (1 - root).square())
    ...
    denominator = input_delta + \
        ((input_derivatives + input_derivatives_plus_one -
         2 * input_delta) * theta_one_minus_theta)
    derivative_numerator = input_delta.pow(2) * (input_derivatives_plus_one * theta.pow(
        2) + 2 * input_delta * theta_one_minus_theta + input_derivatives * (1 - theta).pow(2))
    return input_cum_heights + numerator / denominator, derivative_numerator.log() - 2 * denominator.log()


def sequence_mask(length: Tensor, max_length): return Tensor.arange(
    max_length, dtype=length.dtype, device=length.device).unsqueeze(0) < length.unsqueeze(1)


# duration: [b, 1, t_x], mask: [b, 1, t_y, t_x]
def generate_path(duration: Tensor, mask: Tensor):
    b, _, t_y, t_x = mask.shape
    path = sequence_mask(duration.cumsum(axis=2).reshape(
        b * t_x), t_y).cast(mask.dtype).reshape(b, t_x, t_y)
...
def fused_add_tanh_sigmoid_multiply(input_a: Tensor, input_b: Tensor, n_channels: int):
    ...
    t_act, s_act = in_act[:, :n_channels_int, :].tanh(
    ), in_act[:, n_channels_int:, :].sigmoid()
    return t_act * s_act


def cat_lr(t, left, right): return Tensor.full(get_shape(t), left).cat(
    t, dim=-1).cat(Tensor.full(get_shape(t), right), dim=-1)
...
def split(tensor, split_sizes, dim=0):
    ...
    for size in split_sizes:
        slice_range = [(start, start + size) if j ==
                       dim else None for j in range(len(tensor.shape))]
...
def gather(x, indices, axis):
    indices = (indices < 0).where(
        indices + x.shape[axis], indices).transpose(ax1=axis, ax2=0)
...
def norm_except_dim(v, dim):
    ...
    transposed_v = np.transpose(
        v, (dim,) + tuple(i for i in range(v.ndim) if i != dim))
...
def load_model(symbols, hps, model) -> Synthesizer:
    net_g = Synthesizer(len(symbols), hps.data.filter_length // 2 + 1, hps.train.segment_size //
                        hps.data.hop_length, n_speakers=hps.data.n_speakers, **hps.model)
    ...
    for key, v in saved_state_dict.items():
        ...
        try:
            ...
            if weight_g is not None and weight_v is not None:
                ...
                obj, v = getattr(parent, "weight"), weight_norm(
                    weight_v, weight_g, 0)
            ...
            if not skip and obj.shape == v.shape:
                ...
                logging.error(
                    f"MISMATCH SHAPE IN {key}, {obj.shape} {v.shape}")
...
class TextMapper:  # Based on https://github.com/keithito/tacotron
    def __init__(self, symbols, apply_cleaners=True):
        ...
        self._whitespace_re, self._abbreviations = re.compile(r'\s+'), [(re.compile('\\b%s\\.' % x[0], re.IGNORECASE), x[1]) for x in [('mrs', 'misess'), ('mr', 'mister'), ('dr', 'doctor'), ('st', 'saint'), ('co', 'company'), ('jr', 'junior'), (
            'maj', 'major'), ('gen', 'general'), ('drs', 'doctors'), ('rev', 'reverend'), ('lt', 'lieutenant'), ('hon', 'honorable'), ('sgt', 'sergeant'), ('capt', 'captain'), ('esq', 'esquire'), ('ltd', 'limited'), ('col', 'colonel'), ('ft', 'fort'), ]]
    ...
    def phonemize(self, text, strip=True): return _phonemize(
        self.phonemizer, text, default_separator, strip, 1, False, False)

    def filter_oov(self, text): return "".join(
        list(filter(lambda x: x in self._symbol_to_id, text)))
    def base_english_cleaners(self, text): return self.collapse_whitespace(
    ...
    def transliteration_cleaners(
        self, text): return self.collapse_whitespace(unidecode(text.lower()))

    def cjke_cleaners(self, text): return re.sub(r'([^\.,!\?\-…~])$', r'\1.', re.sub(r'\s+$', '', self.english_to_ipa2(
        text).replace('ɑ', 'a').replace('ɔ', 'o').replace('ɛ', 'e').replace('ɪ', 'i').replace('ʊ', 'u')))

    def cjke_cleaners2(self, text): return re.sub(
        r'([^\.,!\?\-…~])$', r'\1.', re.sub(r'\s+$', '', self.english_to_ipa2(text)))

    def cjks_cleaners(self, text): return re.sub(
        r'([^\.,!\?\-…~])$', r'\1.', re.sub(r'\s+$', '', self.english_to_lazy_ipa(text)))

    def english_to_ipa2(self, text):
        _ipa_to_ipa2 = [(re.compile('%s' % x[0]), x[1])
    ...
    def mark_dark_l(self, text): return re.sub(
    ...
    def english_to_lazy_ipa(self, text):
        _lazy_ipa = [(re.compile('%s' % x[0]), x[1]) for x in [('r', 'ɹ'), ('æ', 'e'), ('ɑ', 'a'), ('ɔ', 'o'),
                                                               ('ð', 'z'), ('θ', 's'), ('ɛ', 'e'), ('ɪ', 'i'), ('ʊ', 'u'), ('ʒ', 'ʥ'), ('ʤ', 'ʥ'), ('ˈ', '↓')]]
        return reduce(lambda t, rx: re.sub(rx[0], rx[1], t), _lazy_ipa, self.english_to_ipa(text))

    def expand_abbreviations(self, text): return reduce(
        lambda t, abbr: re.sub(abbr[0], abbr[1], t), self._abbreviations, text)

    def collapse_whitespace(self, text): return re.sub(
        self._whitespace_re, ' ', text)

    def normalize_numbers(self, text):
        import inflect
        self._inflect = inflect.engine()
        text = re.sub(re.compile(
            r'([0-9][0-9\,]+[0-9])'), self._remove_commas, text)
        text = re.sub(re.compile(r'£([0-9\,]*[0-9]+)'), r'\1 pounds', text)
        text = re.sub(re.compile(
            r'\$([0-9\.\,]*[0-9]+)'), self._expand_dollars, text)
        text = re.sub(re.compile(r'([0-9]+\.[0-9]+)'),
                      self._expand_decimal_point, text)
        text = re.sub(re.compile(
            r'[0-9]+(st|nd|rd|th)'), self._expand_ordinal, text)
        text = re.sub(re.compile(r'[0-9]+'), self._expand_number, text)
        return text

    def _remove_commas(self, m): return m.group(
        1).replace(',', '')  # george won't like this
    ...
    def _expand_dollars(self, m):
        ...
        dollars, cents = int(parts[0]) if parts[0] else 0, int(
            parts[1]) if len(parts) > 1 and parts[1] else 0
    ...
    def _expand_decimal_point(self, m): return m.group(
        1).replace('.', ' point ')

    def _expand_ordinal(
        self, m): return self._inflect.number_to_words(m.group(0))
...
Y_LENGTH_ESTIMATE_SCALARS = {"ljs": 2.8, "vctk": 1.74,
                             "mmts-tts": 1.9, "uma_trilingual": 2.3, "cjks": 3.3, "voistock": 3.1}
...
# parser.add_argument("--text", default="""Hello person. If the code you are contributing isn't some of the highest quality code you've written in your life, either put in the effort to make it great, or don't bother.""", help="Specify the text to synthesize. Default is a greeting message.")
...
class TTS:
  def __init__(self,
    model_to_use="uma_trilingual",
    speaker_id=36,
    out_path="/tmp/tts.wav",
    out_dir=None,
    base_name="test",
    noise_scale=0.667,
    noise_scale_w=0.8,
    length_scale=1.666, #1
    num_channels=1,
    sample_width=2, 
    ...
    self.estimate_max_y_length = estimate_max_y_length
    self.seed = seed
    ...
    if model_has_multiple_speakers:
      print(f"Model has {hps.data.n_speakers} speakers")  # TODO: logging
      ...
      print(f"You selected speaker {speaker_id} (name: {speaker_name})") # TODO: logging

    # NOTE: OG # Load emotions if any. TODO: find an english model with emotions, this is untested atm.
    ...
    else: symbols = ['_'] + list('()[]\{\}/+*^%$#@-_=><`~;:,.!?¡¿—…"«»“” ') + list('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz') + list("ɑɐɒæɓʙβɔɕçɗɖðʤəɘɚɛɜɝɞɟʄɡɠɢʛɦɧħɥʜɨɪʝɭɬɫɮʟɱɯɰŋɳɲɴøɵɸθœɶʘɹɺɾɻʀʁɽʂʃʈʧʉʊʋⱱʌɣɤʍχʎʏʑʐʒʔʡʕʢǀǁǂǃˈˌːˑʼʴʰʱʲʷˠˤ˞↓↑→↗↘'̩'ᵻ")
    ...
    print(f"Loaded model with hps: {hps}")  # TODO: logging
    self.hps = hps
    self.text_mapper = text_mapper
    self.model_has_multiple_speakers = model_has_multiple_speakers
    self.net_g = net_g
    self.emotion_embedding = emotion_embedding

  def text_to_speech(self, text="This is an audio test."):
    # Convert the input text to a tensor.
    #text = text
    if self.model_to_use == "mmts-tts": text = self.text_mapper.filter_oov(text.lower())
    stn_tst = self.text_mapper.get_text(text, self.hps.data.add_blank, self.hps.data.text_cleaners)
    print(f"Converted input text to tensor \"{text}\" -> Tensor({stn_tst.shape}): {stn_tst.numpy()}") #TODO: logging
    x_tst, x_tst_lengths = stn_tst.unsqueeze(0), Tensor([stn_tst.shape[0]], dtype=dtypes.int64)
    sid = Tensor([self.speaker_id], dtype=dtypes.int64) if self.model_has_multiple_speakers else None

    # Perform inference.
    start_time = time.time()
    audio_tensor = self.net_g.infer(x_tst, x_tst_lengths, sid, self.noise_scale, self.length_scale, self.noise_scale_w, emotion_embedding=self.emotion_embedding,
                                   max_y_length_estimate_scale=Y_LENGTH_ESTIMATE_SCALARS[self.model_to_use] if self.estimate_max_y_length else None)[0, 0].realize()
    # TODO: logging
    print(f"Inference took {(time.time() - start_time):.2f}s")

    # Save the audio output.
    audio_data = (np.clip(audio_tensor.numpy(), -1.0, 1.0) * 32767).astype(np.int16)
    self.out_path = Path(self.out_path or Path(self.out_dir)/f"{self.model_to_use}{f'_sid_{self.speaker_id}' if self.model_has_multiple_speakers else ''}_{self.base_name}.wav")
    ...
    print(f"Saved audio output to {self.out_path}")  # TODO: logging
