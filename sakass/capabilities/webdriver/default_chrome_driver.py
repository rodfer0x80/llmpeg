...
from sakass.capabilities.webdriver import Driver
...
class DefaultChromeDriver(Driver):
  ...
  def save_screenshot(self, url: str, path: str) -> None:
    if not path: path = os.path.join(self.cache_dir, f"{curr_date}.png")#tempfile.NamedTemporaryFile(delete=False, suffix='.png').name)
    ...
    self.driver.get(url)
    # NOTE: hack to wait for webpage to load, only works when both WebDriverWaits are called
  ...
  def _enable_automation_options(self):
    ...
    self.options.add_argument("--no-sandbox") # dont touch this breaks user perms
